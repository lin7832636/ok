...
class TclassTree {
     ...
     * @return string
     */
    public function get_ordered_tree_array($myid, $str, $sid = 0, $adds = '', $str_group = '') {
        $retarray = array();
        //一级栏目数组
        $child = $this->get_child($myid);
        if (is_array($child)) {
            ...
            foreach ($child as $id => $value) {
                @extract($value);
                $index = '_'.(65535-$value['listorder']).'_'.$value['create_time'].'_'.$value['id'];
                $retarray[$index] = $value;
                $retarray[$index]["child"] = $this->get_ordered_tree_array($id, '');
            }
        }
        return $retarray;
    }
