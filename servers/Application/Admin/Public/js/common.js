;(function () {
            ...
            $('button.J_ajax_submit_btn').on('click', function (e) {
                ...
                form.ajaxSubmit({
                    ...
                    success: function (data, statusText, xhr, $form) {
                        ...
                        if (data.status === 1) {
                            $('<span class="tips_success">' + data.info + '</span>').fadeIn('slow').delay(1000).fadeOut(function () {
                            }).appendTo(btn.parent());
                        } else if (data.status === 0) {
                            $('<span class="tips_error">' + data.info + '</span>').fadeIn('fast').appendTo(btn.parent());
    ...
    //删除教材
    if ($('a.J_ajax_del_tclass').length) {
        Wind.use('artDialog', function () {
            $('.J_ajax_del_tclass').on('click', function (e) {
                e.preventDefault();
                var $_this = this,
                    $this = $($_this),
                    href = $this.prop('href'),
                ...
                art.dialog({
                    title: false,
                    icon: 'question',
                    content: '当前教材可能包含子教材,此操作将删除当前教材以及当前教材目录下的所有子教材!是否确认删除?',
                    follow: $_this,
                    close: function () {
                        $_this.focus();; //关闭时让触发弹窗的元素获取焦点
                        return true;
    ...
    //所有的开启操作，开启后刷新页面
    if ($('a.J_ajax_open').length) {
        Wind.use('artDialog', function () {
            $('.J_ajax_open').on('click', function (e) {
                e.preventDefault();
                var $_this = this,
                    $this = $($_this),
                    href = $this.prop('href'),
    ...
    //所有的关闭操作，关闭后刷新页面
    if ($('a.J_ajax_close').length) {
        Wind.use('artDialog', function () {
            $('.J_ajax_close').on('click', function (e) {
                e.preventDefault();
                var $_this = this,
                    $this = $($_this),
                    href = $this.prop('href'),
    ...
    if ($('a.J_ajax_status_no').length) {
        Wind.use('artDialog', function () {
            $('.J_ajax_status_no').on('click', function (e) {
                e.preventDefault();
                var $_this = this,
                    $this = $($_this),
                    href = $this.prop('href'),
     ...
     if ($('a.J_ajax_status_yes').length) {
        Wind.use('artDialog', function () {
            $('.J_ajax_status_yes').on('click', function (e) {
                e.preventDefault();
                var $_this = this,
                    $this = $($_this),
                    href = $this.prop('href'),
	 ...
	 if ($('a.J_ajax_nopass').length) {
        Wind.use('artDialog', function () {
            $('.J_ajax_nopass').on('click', function (e) {
                e.preventDefault();
                var $_this = this,
                    $this = $($_this),
                    href = $this.prop('href'),
     ...
     if ($('a.J_ajax_pass').length) {
        Wind.use('artDialog', function () {
            $('.J_ajax_pass').on('click', function (e) {
                e.preventDefault();
                var $_this = this,
                    $this = $($_this),
                    href = $this.prop('href'),
    ...
    //删除预览图片
    if ($('a.J_ajax_del_img').length) {
        Wind.use('artDialog', function () {
            $('.J_ajax_del_img').on('click', function (e) {
                e.preventDefault();
                var $_this = this,
                    $this = $($_this),
                    href = $this.prop('href'),
...
function search_input(){
      $('.search_time_start').on('keyup',function(){
          $(this).val('');
      });
      $('.search_time_end').on('keyup',function(){
          $(this).val('');
      });
      $(".search_telno").keyup(function () {
