...
class Common{
	...
	//U_ 封装调用方法
	function U_getSignCertId(){
		return getSignCertId() ;
	}
	function U_getEncryptCertId(){
		return getEncryptCertId() ;
	}
	function U_encryptData($data){
		return encryptData($data);
	}
	function U_sign($params){
		return sign($params);
	}
	function U_verify($params){
	...
	function create_html($params, $action) {
		//error_log(print_r(C('UNIONPAY',1)));
	...
	function getCustomerInfoStrNew($customerInfo) {
		...
		if(count($encryptedInfo) != 0){
			...
			$encryptedInfo = encryptData ( $encryptedInfo, C('UNIONPAY.SDK_ENCRYPT_CERT_PATH') );
