...
class traverseDir {
    ...
    public function tozip($items) {
        ...
        $zip->open($this->savepath . $zipname . '.zip', ZipArchive::OVERWRITE);
        $zip->addEmptyDir('bukao');
        if (!empty($items['bukao'])) {
            foreach ($items['bukao'] as $val) {
                $val = iconv('utf-8', 'gb2312', $val);
                if (is_file($this->currentdir . '/shoukao/' . $val)) {
                    $zip->addFile($this->currentdir . '/bukao/' . $val, 'bukao/' . $val);
                }
            }
        }
        $zip->addEmptyDir('shoukao');
        if (!empty($items['shoukao'])) {
            foreach ($items['shoukao'] as $val) {
                $val = iconv('utf-8', 'gb2312', $val);
                if (file_exists($this->currentdir . '/shoukao/' . $val)) {
                    $zip->addFile($this->currentdir . '/shoukao/' . $val, 'shoukao/' . $val);
                }
            }
        }
        $zip->close();
        //下载文件
        $dw = new download($zipname . '.zip', $this->savepath);
        $dw->getfiles();
        unlink($this->savepath . $zipname . '.zip');
        $this->remove_files($this->currentdir);
        //var_dump($zip->open($this->savepath.$zipname.'.zip',ZipArchive::OVERWRITE));
        /* die;
    ...
    public function remove_files($path) {
        ...
        $this->remove_base_dir($path);
    }

    public function remove_base_dir($path) {
